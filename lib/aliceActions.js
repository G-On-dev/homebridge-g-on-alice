...
function aliceAction(message, callback) {
  ...
  try {
    ...
    homebridge.HAPaccessories(function(endPoints) {
      ...
      homebridge.HAPcontrolByDeviceID(endPoints[0].instance.deviceID, JSON.stringify(command_body), function(err, status) {
        this.log("Action", endPoints[0].instance.deviceID, JSON.stringify(command_body), status, err);
        if (err) {
