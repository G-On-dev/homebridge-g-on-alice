  ...
  switch (characteristic_data.description) {
    case "Target Heating Cooling State":
      if (!characteristic_data.validValues) {
        ...
        const modes = [];
        for (let i = 0; i < characteristic_data.validValues.length; i++) {
...
function convertHomeBridgeValueToAliceValue(capability_data, characteristic_data, _characteristics) {
  const characteristics = Object.keys(_characteristics).map((key) => _characteristics[key]);
  const converted_capability_data = {
